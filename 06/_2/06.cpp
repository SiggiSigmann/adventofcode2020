...
/*##############################################################################
#count how many same answers are given per group and store this number in answers
##############################################################################*/
	...
	for(int i = 0; i < groupNumber; i++){
		...
		for(int j = 0; j < maxGroupSize; j++){
			...
			if(!person.empty()){
				...
				//calulate answeres per person
				int answerePerPerson = 0;
				if(!person.empty()){
					for(int k = 0; k < person.length(); k++){
						int anserID = (int)pow(2, person.at(k)-97);
						answerePerPerson |= anserID;
					}

				}

				//remove differen anseres form group
				givenAnsweres &= answerePerPerson;
			}
		}

		//count 1's in givenAnsweres
		int onesInInt = 0;
		while(givenAnsweres){
			onesInInt += givenAnsweres & 1;
			givenAnsweres >>= 1;
		}
		
		answeres[i] = onesInInt;
	}
